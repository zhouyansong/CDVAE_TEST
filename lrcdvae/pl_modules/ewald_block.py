...
from lrcdvae.common.scale_factor import ScaleFactor
...
class EwaldBlock(torch.nn.Module):
    ...
    def forward(
        ...
        dtype = hres.dtype  # 获取 hres 的数据类型
        ...
        # 计算 cos 和 sin，并显式转换为 hres 的类型
        cos_term = (sinc_damping * torch.cos(dot)).to(dtype=dtype)
        sin_term = (sinc_damping * torch.sin(dot)).to(dtype=dtype)
