...
import os
import sys
script_dir = os.path.dirname(os.path.abspath(__file__)) #新增的通用执行运行路径
...
from lrcdvae.common.utils import log_hyperparameters, PROJECT_ROOT
...
def build_callbacks(cfg: DictConfig) -> List[Callback]:
    ...
    if "model_checkpoints" in cfg.train:
        ...
        callbacks.append(
            ...
            )  # 新增三个参数 every_n_epochs, save_last, filename 
        )
...
def run(cfg: DictConfig) -> None:
    ...
    # Hydra run directory
    hydra_dir = Path(HydraConfig.get().run.dir) #通过传入的conf路径
