...
def validate_structure_connectivity(structure, exclude_elements=['C', 'B'], search_cutoff=8.0, max_bond=4.0, min_bond=1.4, min_neighbors=1):
    structure_elements = set([site.specie.symbol for site in structure])
    target_elements_in_structure = structure_elements.intersection(set(exclude_elements))
    if len(target_elements_in_structure) > 1 or 'H' in structure_elements:
        element_combinations = sorted(list(target_elements_in_structure))
        validation_info = {
            ...
            'rejection_reason': f'Contains multiple target elements: {element_combinations}',
            'structure_elements': list(structure_elements),
            'target_elements_found': element_combinations
        }
    ...
    validation_info = {
        ...
        'structure_elements': list(structure_elements),
        'target_elements_found': list(target_elements_in_structure),
        'rejection_reason': None if is_valid else 'Connectivity issues'}
...
if __name__ == "__main__":
    ...
    for idx,row in df.iterrows():
        ...
        #     continue
        try:
            file = rootdir / f'{uid}.vasp'
            struct = Structure.from_file(file)
            total_atoms = len(struct)
            if total_atoms > 50:
                print(f"Skipping {uid} due to too many atoms: {total_atoms}")
                invalid_data.append({'material_id': uid, 'formation_energy_per_atom': row['hform'], 'error': 'Too many atoms'})
                continue
            ...
            if not is_valid:
                print(f"Skipping {uid} due to : {validation_info['rejection_reason']}")
                invalid_data.append({
                    'material_id': uid,
                    'formation_energy_per_atom': row['hform'],
                    'error': f"Rejection reason: {validation_info['rejection_reason']}",
            ...
            cif_data.append({'material_id': uid, 'cif': cif_string, 
                             'formation_energy_per_atom': row['hform'],'numbers': total_atoms, 
                             'closest_neighbor_distances': validation_info['closest_neighbor_distances']})
        
        except Exception as e:
            print(f"Error processing {uid}: {e}")
            invalid_data.append({'material_id': uid, 'formation_energy_per_atom': row['hform'], 
                                 'error': str(e)})
